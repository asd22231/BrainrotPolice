...
function VeilUI.new(config)
    ...
    sidebar.Size = UDim2.new(0, 180, 1, -46)   -- wider
    ...
    sidebarLayout.VerticalAlignment = Enum.VerticalAlignment.Top   -- critical
    sidebarLayout.FillDirection = Enum.FillDirection.Vertical
    padding(sidebar, 8, 8, 8, 8)   -- small padding
    ...
    content.Position = UDim2.new(0, 180, 0, 46)
    ...
    pages.Name = "Pages"
    pages.Size = UDim2.new(1, -24, 1, -20)
...
-- ========================================================
-- COMPONENTS (unchanged – Toggle, Button, Slider, Dropdown, TextBox, Label)
-- ========================================================
...
-- Fallback game list (so the Games tab is never empty)
local FALLBACK_GAMES = {
    { game = "Example Game 1", id = 1234567890, status = "online" },
    { game = "Example Game 2", id = 9876543210, status = "offline" },
}
...
if listSuccess and type(listData) == "table" and #listData > 0 then
    gameList = listData
    print("Loaded " .. #gameList .. " games from remote.")
else
    gameList = FALLBACK_GAMES
    warn("Using fallback game list (remote fetch failed or empty).")
end
...
-- Ensure the sidebar layout is correctly aligned (redundant but safe)
...
-- Add game buttons from the list (fallback or remote)
...
-- If we are in a supported game, load its specific script
