...
local TeleportService = game:GetService("TeleportService") -- for teleporting
...
function VeilUI.new(config)
    ...
    local size = config.Size or {700, 500}  -- increased default size
    ...
    main.Name = "Main"
    main.Size = UDim2.new(0, size[1], 0, size[2])  -- set directly, no animation to avoid squashing
    ...
    sidebar.Size = UDim2.new(0, 170, 1, -46)  -- slightly wider
    sidebar.Position = UDim2.new(0, 0, 0, 46)
    sidebar.BackgroundColor3 = Palette.Surface
    sidebar.BorderSizePixel = 0
    sidebar.ZIndex = 10
    sidebar.ClipsDescendants = false  -- prevent clipping
    sidebar.Parent = main

    -- Sidebar layout: left-aligned, top, vertical
    ...
    pages.Name = "Pages"
    pages.Size = UDim2.new(1, -24, 1, -20)   -- less padding
    pages.Position = UDim2.new(0, 12, 0, 10)
    ...
    -- Pulse glow (still have a small effect)
    task.spawn(function()
...
function VeilUI:CreateTab(name, icon)
    ...
    pageLayout.HorizontalAlignment = Enum.HorizontalAlignment.Left   -- left-aligned for better fit
...
-- ========================================================
-- GAME SCRIPT LOADER (FULLY FIXED)
-- ========================================================

-- Helper to get raw content from GitHub (REPLACE WITH YOUR ACTUAL REPO)
local function getgitpath(folder)
    return "https://raw.githubusercontent.com/yourusername/yourrepo/main/" .. folder .. "/"
end
...
if listSuccess then
    ...
    warn("Failed to load game list, using empty.")
end

-- Create the UI with dynamic sizing
local viewport = workspace.CurrentCamera.ViewportSize
local ui = VeilUI.new({
    Name = "BrainrotPolice",
    Size = {math.min(viewport.X * 0.7, 800), math.min(viewport.Y * 0.65, 600)},
})

-- Note: sidebar and content sizes are already adjusted inside the class (170px sidebar)
-- But if you want to tweak further, you can do it here.

-- Wait a tiny bit for everything to settle
task.wait(0.1)

-- Create the Games tab (always visible)
local gamesTab = ui:CreateTab("Games")

-- Add game buttons from the list
for _, v in ipairs(gameList) do
    ui:CreateButton(gamesTab, v["game"] .. " [" .. (v["status"] or "unknown") .. "]", function()
        if v["id"] then
            -- Use TeleportService to join the game
            TeleportService:Teleport(v["id"])
        end
    end)
end

-- If we are currently in a supported game, load its specific script
if game.PlaceId then
    for _, v in ipairs(gameList) do
        if tostring(game.PlaceId) == tostring(v["id"]) then
            local scriptUrl = getgitpath("games") .. v["id"] .. ".lua"
            local scriptSuccess, scriptData = pcall(function()
                return game:HttpGet(scriptUrl)
            end)
            if scriptSuccess then
                local gameScript = loadstring(scriptData)
                if gameScript then
                    local gameTab = ui:CreateTab(v["game"])
                    gameScript()(gameTab, ui)  -- pass tab and ui reference
                else
                    warn("Failed to compile script for " .. v["game"])
                end
            else
                warn("Failed to load script for " .. v["game"])
            end
            break
        end
    end
