...
function VeilUI.new(config)
    ...
    pages.Name = "Pages"
    pages.Size = UDim2.new(1, -32, 1, -24)
    pages.Position = UDim2.new(0, 20, 0, 12)
...
-- Adjust sidebar and content after creation (or modify the class)
...
pages.Size = UDim2.new(1, -16, 1, -16)
pages.Position = UDim2.new(0, 8, 0, 8)

-- Make the page layouts left-aligned (optional)
...
-- Load current game script if in supported game
if game.PlaceId then
    for i, v in pairs(gameList) do
        if tostring(game.PlaceId) == tostring(v["id"]) then
            local gameScript = loadstring(game:HttpGet(getgitpath("games") .. v["id"] .. ".lua"))
            if gameScript then
